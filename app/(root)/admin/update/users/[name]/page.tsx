...
const CreateUsers = () => {
  ...
  const [currentUser, setCurrentUser] = useState<formDataType>({
    name: "",
    role: "WORKER",
    gender: "FEMALE",
    phone: "",
  ...
  async function onSubmit(data: formDataType) {
    console.log(data)
    // @ts-ignore
    Object.assign(data, {id: currentUser.id})
    const updatedUser = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/users/update`,{
      method: "Post",
  ...
  async function findUserByUserName(name: string) {
    const club = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/users/findOne`, {
      method: 'POST',
      headers: {"Content-Type": "application/json"},
  ...
  async function handleDeleteUser(id: number){
    const deletedUser = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/users/delete`)
  }
  ...
  useEffect(() => {
    ...
    methods.reset({
      ...
      // @ts-ignore
      miniClubName: (currentUser?.miniClub?.name)?(currentUser?.miniClub?.name):"",
      // @ts-ignore
      clubName: (currentUser?.club?.name)?(currentUser?.club?.name):"",
      // @ts-ignore
      positionName: (currentUser?.position?.name)?(currentUser?.position?.name):""
    })
  }, [currentUser]);
      ...
      <div className="bg-white rounded-lg shadow-sm p-8">
        ...
        <form onSubmit={methods.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <div className="flex justify-between mt-8">
            {/*@ts-ignore*/}
            <Button color="danger" startContent={<UserIcon />} variant="shadow" onPressEnd={()=>{}}>
              删除用户
            </Button>
            <Button
              type="submit"
